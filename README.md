...
## Автор решения
Волков Александр Юрьевич, tg: @hayth4m

## Описание реализации
Программа реализована на языке программирования Python.
Для работы были использованы встроенные в Python библиотеки datetime и json, поэтому код автономен.
Программа состоит из 3 основных функций, каждая из которых выполняет свою часть задачи. 
Функция load_json принимает путь к JSON-файлу в качестве аргумента и возвращает данные о заказах из этого файла в виде словаря.

Функция get_popular анализирует данные о заказах и определяет категории товаров, заказанных в период с 1 по 31 декабря. 
Для этого происходит перебор заказов, проверка даты заказа и подсчет количества товаров в каждой категории.

Функция generate_report вызывает две предыдущих функции, а затем выводит необходимый нам результат в виде строки в формате json.

## Инструкция по сборке и запуску решения
Чтобы запустить программу требуется установленный Python на компьютере, 
json файл должен находится в той же папке, что и скрипт main.py
Запуск возможен через любую IDE или консоль, для этого необходимо перейти по пути где лежит скрипт и json и ввести:
python main.py
после выполнения в консоль будет выведен ответ в формате строки json.
